...
class ApiAllowed
{
    ...
    public function handle(Request $request, Closure $next, ...$allowed_roles): Response
    {
        ...
        if (!auth()->check()) return error(401, "Nicht authorisiert");
        if (!$user = auth()->user()) return error(401, "Nicht authorisiert");

        if (!$this->userHasRole($allowed_roles)) return error(403, "Unzulässig");
